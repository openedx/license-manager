...
def populate_product(apps, schema_editor):
    ...
    for plan in SubscriptionPlan.objects.all().iterator():
        ...
        if ns_id:
            ...
            if product_to_associate:
                plan.product = product_to_associate
                plan.save()
            else:
                product_to_associate = products_with_plan_type[0]

                # If the plan wasn't associated to a product by Netsuite Id, the Netsuite Id is invalid. Associate it by plan type at this point.
                # If multiple products are associated with a plan type, just use the first one (will only happen in a non prod environment).
                plan.product = product_to_associate
                plan.save()
